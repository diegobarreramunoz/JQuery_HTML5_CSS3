...
g_l["YEAR_FWD"] = "Un ano adelante";
g_l["YEAR_BCK"] = "Un ano atras";
...
/*
	...
	Thank you!
*/
g_jsDatePickImagePath = "../styles/img/";
...
JsDatePick.prototype.setConfiguration = function(aConf){
	...
	this.oConfiguration.yearsRange		= (aConf["yearsRange"] != null) ? aConf["yearsRange"] : [1753,9999];
	...
	this.oConfiguration.button			= (aConf["button"] != null) ? aConf["button"] : aConf["target"];
...
JsDatePick.prototype.populateFieldWithSelectedDate = function(){
	jq("#"+this.oConfiguration.target).val(this.getSelectedDayFormatted());
	jq("#"+this.oConfiguration.target).removeClass( "validation" );
...
JsDatePick.prototype.makeCalendar = function(){
	...
	if (this.oConfiguration.useMode == 2){
		if (this.oConfiguration.target != false){
			if (typeof(jq("#"+this.oConfiguration.target)) != null){
				inputElement = document.getElementById(this.oConfiguration.target);
				buttonElement = document.getElementById(this.oConfiguration.button);
				
				aSpan = document.createElement("span");
				buttonElement.parentNode.replaceChild(aSpan,buttonElement);
				aSpan.appendChild(buttonElement);
		
				buttonElement.setAttribute("globalNumber",this.globalNumber);
				buttonElement.onclick = function(){ JsDatePick.getCalInstanceById(this.getAttribute("globalNumber")).showCalendar(); };
				buttonElement.onfocus = function(){ JsDatePick.getCalInstanceById(this.getAttribute("globalNumber")).showCalendar(); };
		...
		if (this.oConfiguration.button != null){
			jq("#"+this.oConfiguration.button).append(JsDatePickBox);
			jq("#"+this.oConfiguration.button).css("position","relative");
			JsDatePickBox.style.position = "absolute";
...
JsDatePick.prototype.determineFieldDate = function(){
	...
	if (this.lastPickedDateObject){
		...
		aField = jq("#"+this.oConfiguration.target);
		
		if (jq.trim(aField.val()).length == 0){
			this.unsetSelection();
...
JsDatePick.prototype.showCalendar = function(){
	if (this.JsDatePickBox.style.display == "none" && !jq('#'+this.oConfiguration.target).is(':disabled')){
		this.determineFieldDate();
		this.JsDatePickBox.style.display = "block";
		this.resizeCalendar();
		this.executePopulationDelegateIfExists();
		jq(this.JsDatePickBox).mouseover(function(){ document.onclick=function(){}; });
		jq(this.JsDatePickBox).attr("globalCalNumber", this.globalNumber);
		jq(this.JsDatePickBox).mouseout(function(){
...
JsDatePick.prototype.setDaySelection = function(anElement){
	...
	if  (this.flag_DayMarkedBeforeRepopulation){
		...
		jq(this.lastMarkedDayObject).attr("isSelected",0);
	...
	jq(anElement).attr("isSelected",1);
...
JsDatePick.prototype.getDOMControlBar = function(){
	...
	jq(controlsBar).attr("globalNumber",this.globalNumber);
	jq(monthForwardButton).attr("globalNumber",this.globalNumber);
	jq(monthBackwardButton).attr("globalNumber",this.globalNumber);
	jq(yearBackwardButton).attr("globalNumber",this.globalNumber);
	jq(yearForwardButton).attr("globalNumber",this.globalNumber);
